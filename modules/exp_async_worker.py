...
def worker():
    ...
    def load_models_and_process_prompts(async_task, refiner_model_name, base_model_name, loras, base_model_additional_loras, use_synthetic_refiner):
    ...
    def handler(async_task):
        ...
        # Process the initial loras list
        loras = [[str(args.pop()), float(args.pop()), bool(args.pop())] for _ in range(modules.config.default_loras_max_number)]
        print(f"Original loras: {loras}")
        loras = apply_enable_loras(loras)
        print(f"Processed loras: {loras}")

        # Backup the filtered loras before any additional default loras are added
        all_loras = [lora for lora in loras if lora[0] != "None"]
        print(f"Backup all_loras: {all_loras}")

        # Temporarily use loras as a placeholder for potential default loras
        loras = []
        ...
        isStoped = False
        index_total_task = 0
        evaluated_total_task = 0

        base_model_names = [base_model_name]
        if enable_test_base_model_mode:
            base_model_names = [name for name in modules.config.model_filenames if "SDXL" in name]
            
        evaluated_total_task += len(base_model_names)
        ...
        for base_model_name in base_model_names:
            ...
            print(f"base_model_names: {base_model_names}")
            
            refiner_model_names = [refiner_model_name]
            if enable_test_refiner_model_mode:
                refiner_model_names = [name for name in modules.config.model_filenames if "SD" in name and base_model_name != name]
            
            evaluated_total_task += len(refiner_model_names)
            
            for refiner_model_name in refiner_model_names:
                if isStoped:
                    print(f"isStoped")
                    break
                
                print(f"refiner_model_name: {refiner_model_name}")
                ...
                selected_loras_by_test = []
                
                if enable_test_loras_mode:
                    # In test mode, for each test from all loras
                    all_loras.insert(0, ["None", 1.0])
                    image_number = 1
                    print(f"Adjusted image_number: {image_number}")
                    selected_loras_by_test = [[lora] for lora in all_loras]
                else:
                    # In normal mode, only one test with all loras
                    selected_loras_by_test.append(all_loras)
                    
                print(f"all_loras: {all_loras}")
                print(f"selected_loras_by_test: {selected_loras_by_test}")
                
                default_loras = loras  # Use the temporary loras as default loras
                for selected_loras in selected_loras_by_test:
                    if isStoped:
                        print(f"isStoped")
                        break
                    
                    print(f"selected_loras: {selected_loras}")
                    loras = selected_loras + default_loras
                    print(f"final loras: {loras}")
                        
                    load_models_and_process_prompts(async_task, refiner_model_name=refiner_model_name, base_model_name=base_model_name,
                                                    loras=loras, base_model_additional_loras=base_model_additional_loras,
                                                    use_synthetic_refiner=use_synthetic_refiner)
                    
                    for img_index in range(image_number):
                        
                        evaluated_total_task += image_number
                        
                        if skip_prompt_processing:
                            continue
                        
                        task_seed = (seed + img_index) % (constants.MAX_SEED + 1)  # randint is inclusive, % is not
                        ...
                        if use_expansion:
                            progressbar(async_task, 5, f'Preparing Fooocus text #{index_total_task + 1} ...')
                        ...
                        progressbar(async_task, 7, f'Encoding positive #{index_total_task + 1} ...')
                        task['c'] = pipeline.clip_encode(texts=task['positive'], pool_top_k=task['positive_top_k'])

                        if abs(float(cfg_scale) - 1.0) < 1e-4:
                            task['uc'] = pipeline.clone_cond(task['c'])
                        else:
                            progressbar(async_task, 10, f'Encoding negative #{index_total_task + 1} ...')
                        ...
                        def callback(step, x0, x, total_steps, y):
                            done_steps = index_total_task * steps + step
                            async_task.yields.append(['preview', (
                                int(15.0 + 85.0 * float(done_steps) / float(all_steps)),
                                f'Step {step}/{total_steps} in the {index_total_task + 1}-th Sampling on {evaluated_total_task}',
                                y)])
                        ...
                        try:
                            ...
                            for x in imgs:
                                d = [
                                    ...
                                    ('Base Model', task['base_model_name']),
                                    ('Refiner Model', task['refiner_model_name']),
                                    ('Refiner Switch', refiner_switch),
                                    ('Sampler', sampler_name),
                                    ('Scheduler', scheduler_name),
                                    ('Seed', task['task_seed']),
                                ]
                                for li, (n, w) in enumerate(task['loras']):
                                    if n != 'None':
                            ...
                            yield_result(async_task, imgs, do_not_show_finished_images=do_not_show_finished_images)
